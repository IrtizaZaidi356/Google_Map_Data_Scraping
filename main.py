...
def parse_address_parts(address: str):
    # Clean weird unicode; keep ASCII so regex is predictable
    cleaned = re.sub(r'[^\x00-\x7F]+', '', (address or "")).strip()
    parts = [p.strip() for p in cleaned.split(',') if p.strip()]
    city, zip_code, country = "-", "-", "-"

    # Country guess: the last chunk is commonly the country name
    if len(parts) >= 1:
        country = parts[-1]

    # The second-last chunk often contains city and maybe ZIP/state
    if len(parts) >= 2:
        second_last = parts[-2]
        # Pull a 4–6 digit ZIP if present
        possible_zip = re.findall(r'\\b\\d{4,6}\\b', second_last)
        if possible_zip:
            zip_code = possible_zip[0]
            # Remove ZIP from the city text
            city = second_last.replace(zip_code, "").strip(", ").strip()
        else:
            city = second_last
...
def extract_place(page: Page, user_input: str, index: int, listing_url: str) -> Place:
    ...
    try:
        m=re.search(r'@([0-9\.-]+),([0-9\.-]+)',page.url)
        if m:
